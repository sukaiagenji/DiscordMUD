var settings, character, longdesc, currentAt, roomId, info, exitList, tempRoom;
var intervals = [];
var gaemcmds = [];
var gaemcmdsStack = [];
var world = [];
...
expFuncs = {
	"checkCmd": function (bot,msg) {
		...
		if (cmdTxt == "help") {
			for (var i in gaemcmds) {
				for (var cmd in gaemcmds[i]) {
					var info = "." + cmd;
					var usage = gaemcmds[i][cmd].usage;
					if (usage) {
						info += " " + usage;
					}
					var description = gaemcmds[i][cmd].description;
					if(description){
						info += "\n\t" + description;
					}
	...
	"fn300sec": function () {
			require("fs").writeFile(charsFile,JSON.stringify(character,null,4), function (e) {
				if (e) {
					...
					console.log("Auto-saved character file.");
				}
			});
		}
}
...
expCmds = {
    "newgaem": {
        ...
        process: function(bot,msg,suffix) {
			if (character[msg.author.id] && character[msg.author.id].roomName) {
			...
			if (msg.author.username.toLowerCase().replace(/[^a-z0-9]/gi,'').trim() == "") {
			...
			setTimeout(function () {
				for (var i = 0; i < bot.channels.length; i++) {
					if (!suffix) {
						if (bot.channels[i].name == msg.author.username.toLowerCase() || (character[msg.author.id] && bot.channels[i].name == character[msg.author.id].roomName)) {
	...
	"savegaem": {
		...
		process: function (bot,msg,suffix) {
			...
			for (var i in world) {
				require("fs").writeFile(settings.botLocation + "/gaemfiles/world/" + i + ".wrld.json",JSON.stringify(world[i],null,4), function (e) {
					if (e) {
						console.log("Error writing world file: " + e);
						bot.sendMessage(msg.channel, msg.author + " There was a problem saving the world files!!! D:");
					} else {
						bot.sendMessage(msg.channel, msg.author + " Your world files were saved!");
					}
				});
			}
		}
